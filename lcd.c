...
void goto_XY(t_display_port *lcd, unsigned char x, unsigned char y){
    ...
    else if(x == 3)         //verifica se é segunda linha
    {
       temp = 0x90 + y - 1; //posição inicial da segunda linha mais coluna, menos 1
       z = temp>>4;         //pegando bits mais significativos
       a = temp;            ////pegando bit menos significativos
       lcd_cmd(lcd, z);
       __delay_ms(1);
    ...
    else if(x == 4)         //verifica se é segunda linha
    {
       temp = 0xD0 + y - 1; //posição inicial da segunda linha mais coluna, menos 1
       z = temp>>4;         //pegando bits mais significativos
       a = temp;            ////pegando bit menos significativos
       lcd_cmd(lcd, z);
       __delay_ms(1);
